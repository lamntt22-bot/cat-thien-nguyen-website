  ...
  return (
    <header className="relative z-30 border-b border-gold-500/30 bg-maroon-900/97 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-2.5 sm:px-8">
        ...
        <nav className="hidden items-center gap-4 text-sm font-medium text-cream-100/85 lg:flex">
          {NAV_LINKS.map((link) => (
        ...
        <div className="hidden lg:block">
          <button
            type="button"
            onClick={() => open()}
            className="whitespace-nowrap rounded-full bg-gold-500 px-5 py-2.5 text-sm font-bold text-maroon-950 shadow-md shadow-gold-500/20 transition hover:bg-gold-400"
          >
            Đăng ký nhận ưu đãi
          </button>
        </div>
        ...
        <button
          ...
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-cream-50 lg:hidden"
        >
